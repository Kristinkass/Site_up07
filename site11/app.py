...
def view_country(country_id):
    conn = mysql.connector.connect(**db_config)
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT name FROM Countries WHERE country_id = %s", (country_id,))
    country = cursor.fetchone()
    conn.close()
    ...
    return country_page(country['name'])
...
def country_page(country_name):
    ...
    return render_template('шаблон20.html', **country_data)
